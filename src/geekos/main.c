...
void perform(ulong_t arg){
	Print("Hello from nitish\n");
	while(1){
		Keycode a = Wait_For_Key();
		if(a >= 32768) continue;
		Print("%c", a);
	}
}
...
//    Init_Screen();
    ...
    Set_Current_Attr(ATTRIB(BLACK, GREEN));
...
//	struct Kernel_Thread *thre = Start_Kernel_Thread(perform, 0, PRIORITY_NORMAL, false, "Performer"); 
//	Join(thre); 
